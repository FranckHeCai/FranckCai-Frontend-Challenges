...
const updateUI = (type) =>{
  result.innerHTML= `
   <h3 class="text-xl font-medium text-white mb-1">Your results</h3>
        ...
        <div class="p-6 bg-[var(--slate-dark)] rounded-3xl border-t-3 border-[var(--lime)]">
          <div>
            <p class="text-white/50 mb-1">Your monthly repayments</p>
            <p class="text-5xl text-[var(--lime)] font-bold">€${monthlyPayment.toFixed(2)}</p>
          </div>
          <div class="mt-6 pt-6 border-t-3 border-white/20">
            <p class="text-white/50">Total you'll repay over the term</p>
            <p class="text-white text-xl font-bold">€${totalPayment.toFixed(2)}</p>
          </div>
        </div>
  `
}
